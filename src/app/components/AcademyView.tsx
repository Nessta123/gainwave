...
export default function AcademyView({ darkMode, userData, updateUserBalance }: any) {
  ...
  // Tukaj razširimo tip, da podpira morebitni image_url iz academyData
  const activeLesson: any = academyData[currentLevel]?.lessons[currentLessonIdx];
        ...
        <div className={`p-8 md:p-12 rounded-[2.5rem] h-full overflow-y-auto custom-scrollbar relative ${getGlassPanelClass(darkMode)}`}>
          ...
          {isAcademyCompleted ? (
            ...
            <div className="animate-in slide-in-from-right-4 duration-500 pb-10">
              ...
              {/* 🔥 DINAMIČNI PRIKAZ NEONSKIH SLIK IZ UPORABNIKOVE MAPE 🔥 */}
              {activeLesson.image_url && (
                <div className="my-6 p-[2px] rounded-2xl bg-gradient-to-r from-blue-500/20 via-purple-500/20 to-pink-500/20 backdrop-blur-md border border-white/5 shadow-[0_15px_30px_rgba(0,0,0,0.5)] overflow-hidden">
                  <img 
                    src={activeLesson.image_url} 
                    alt={activeLesson.title} 
                    className="w-full h-auto max-h-[400px] object-cover rounded-2xl group-hover:scale-[1.02] transition-transform duration-500"
                  />
                  <div className="p-3 bg-zinc-950/40 border-t border-white/5 text-center">
                    <p className="text-[9px] font-black uppercase tracking-[0.2em] text-blue-400 animate-pulse">
                      ⚡ TECHNICAL INFRASTRUCTURE SCHEMATIC
                    </p>
                  </div>
                </div>
              )}
